...
This module provides the main Client class that handles HTTP requests
with adaptive rate limiting, request history tracking, and streaming capabilities.
"""

from typing import Optional, Dict, Any, Union, Tuple, Type, TypeVar
...
class Client:
    ...
    This client automatically detects and respects rate limits by:
    1. Waiting for the first rate limit refusal (HTTP 429)
    2. Initializing Bayesian priors based on domain knowledge and observed request patterns
    3. Continuously updating estimates with each subsequent request using variance-based weighting
    4. Throttling requests according to the learned rate limits with conservative bounds
    5. Adaptively restarting estimation if rate limits change, with principled uncertainty handling
    """
    def __init__(self, 
                 base_url: Optional[str] = None,
    ...
    def request(self, 
        ...
        """
        Make an HTTP request with adaptive Bayesian rate limiting.
    ...
    async def async_request(self,
        ...
        """
        Make an asynchronous HTTP request with adaptive Bayesian rate limiting.
